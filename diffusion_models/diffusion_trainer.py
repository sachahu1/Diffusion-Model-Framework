...
class DiffusionTrainer:
  def __init__(
    ...
    self.checkpoint_path = pathlib.Path(f"../checkpoints") / self.training_configuration.training_name
  ...
  def log_to_tensorboard(self, metrics: Dict[str, float], global_step: int):
    ...
    if (global_step % self.log_configuration.image_rate == 0) and (self.log_configuration.number_of_images > 0):
      image_channels, image_height, image_width = self._image_shape
      images = torch.randn(
        (self.log_configuration.number_of_images, image_channels, image_height, image_width),
        device=self.device
      )
